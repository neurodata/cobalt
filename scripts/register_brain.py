...
def register_data(rmt, config):
    ...
    resolution_atlas = int(params['resolution'])
    ...
    if outdir is None:
        outdir = './process_folder/{}_{}_{}_reg/'.format(shared_params['collection'], shared_params['experiment'], params['channel'])
...
#    if modality.lower() == 'colm': img_p = preprocessor.remove_grid_artifact(img_p, z_axis=2,)
    ...
    anno_channel = 'atlas_{}umreg'.format(resolution_atlas)
...
#    size_r = (y_size, z_size, x_size)
    ...
    img_t = ndreg.imgApplyAffine(anno_10, trans, spacing=spacing.tolist(), useNearest=True)

    print("applying displacement field transformation to atlas labels")
    img_t = ndreg.imgApplyField(img_t, field, spacing=spacing.tolist(), 
                                            size=size_r.tolist(), useNearest=True)
    # reorient annotations to match image
    print("reorienting the labels to match the image")
    img_t = preprocessor.imgReorient(img_t, atlas_orientation, params['orientation'])
#    coll_reg = 'cell_detection'
    ...
    img_t = sitk.GetArrayFromImage(img_t)
    if img_t.dtype != 'uint64':
        img_t = img_t.astype('uint64')
    upload_to_boss(rmt, img_t, ch_rsc_anno)
    viz_link = get_reg_viz_link(shared_params['collection'], shared_params['experiment'], source_channel, anno_channel)
    return viz_link
